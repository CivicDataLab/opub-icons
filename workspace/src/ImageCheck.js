import React from 'react'

export function ImageCheck(props) {
	return (
		<svg
			xmlnsXlink="http://www.w3.org/1999/xlink"
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 18 18"
			width={props.width || 18}
			{...props}
		>
			<rect fill="currentColor" opacity={0} />
			<circle cx="11.9" cy="6.3" r="1.25" />
			<path d="M7.35,13.5a6.119,6.119,0,0,1,.631-2.7C6.927,9.621,5.828,8,4.892,8,3.5565,8,1,12,1,12V3H17V8.4465a6.13939,6.13939,0,0,1,1,.8715V2.5a.534.534,0,0,0-.5625-.5H.5625A.534.534,0,0,0,0,2.5v13a.5345.5345,0,0,0,.5625.5h7.322A6.122,6.122,0,0,1,7.35,13.5Z" />
			<path d="M13.5,9.05a4.45,4.45,0,1,0,4.45,4.45A4.45,4.45,0,0,0,13.5,9.05Zm-1.169,7.156-2.064-2.064a.25.25,0,0,1,0-.3535l.518-.518a.25.25,0,0,1,.3535,0L12.504,14.636l3.053-3.053a.25.25,0,0,1,.3535,0l.5215.5215a.25.25,0,0,1,0,.3535l-3.75,3.75A.25.25,0,0,1,12.331,16.206Z" />
		</svg>
	)
}
