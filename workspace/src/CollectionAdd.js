import React from 'react'

export function CollectionAdd(props) {
	return (
		<svg
			xmlnsXlink="http://www.w3.org/1999/xlink"
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 18 18"
			width={props.width || 18}
			{...props}
		>
			<rect fill="currentColor" opacity={0} />
			<path d="M9.05,12.5A4.45,4.45,0,1,0,13.5,8.05,4.45,4.45,0,0,0,9.05,12.5ZM11,12.25a.25.25,0,0,1,.25-.25H13V10.25a.25.25,0,0,1,.25-.25h.5a.25.25,0,0,1,.25.25V12h1.75a.25.25,0,0,1,.25.25v.5a.25.25,0,0,1-.25.25H14v1.75a.25.25,0,0,1-.25.25h-.5a.25.25,0,0,1-.25-.25V13H11.25a.25.25,0,0,1-.25-.25Z" />
			<path d="M7.542,14H7V9H8.4465A6.184,6.184,0,0,1,9.318,8H7V3h4V6.8845a6.09968,6.09968,0,0,1,1-.3425V3h4V6.8845a6.13724,6.13724,0,0,1,1,.562V2.5a.5.5,0,0,0-.5-.5H1.5a.5.5,0,0,0-.5.5v12a.5.5,0,0,0,.5.5H7.8845A6.09968,6.09968,0,0,1,7.542,14ZM6,14H2V9H6ZM6,8H2V3H6Z" />
		</svg>
	)
}
