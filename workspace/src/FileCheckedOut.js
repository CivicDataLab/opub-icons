import React from 'react'

export function FileCheckedOut(props) {
	return (
		<svg
			xmlnsXlink="http://www.w3.org/1999/xlink"
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 18 18"
			width={props.width || 18}
			{...props}
		>
			<rect fill="currentColor" opacity={0} />
			<path d="M7.875,13.5a5.625,5.625,0,0,1,5.62493-5.625H13.5c.16883,0,.335.01068.5.02527V5H9.5A.5.5,0,0,1,9,4.5V0H2.5A.5.5,0,0,0,2,.5v15a.5.5,0,0,0,.5.5H8.46661A5.59193,5.59193,0,0,1,7.875,13.5Z" />
			<path d="M13.5,9A4.5,4.5,0,1,0,18,13.5,4.5,4.5,0,0,0,13.5,9ZM16,14.4071a.25.25,0,0,1-.42676.17676L14.525,13.53564l-2.31794,2.31793a.25.25,0,0,1-.3535,0l-.70715-.70714a.25.25,0,0,1,0-.35351L13.46436,12.475l-1.04828-1.04821A.25.25,0,0,1,12.59289,11h3.26386A.14327.14327,0,0,1,16,11.14325Z" />
			<path d="M10,0h.043a.5.5,0,0,1,.353.146L13.854,3.6A.5.5,0,0,1,14,3.957V4H10Z" />
		</svg>
	)
}
