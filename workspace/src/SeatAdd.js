import React from 'react'

export function SeatAdd(props) {
	return (
		<svg
			xmlnsXlink="http://www.w3.org/1999/xlink"
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 18 18"
			width={props.width || 18}
			{...props}
		>
			<rect fill="currentColor" opacity={0} />
			<path d="M2.5,9H2a1,1,0,0,0-1,1v6.5a.5.5,0,0,0,.5.5h1a.5.5,0,0,0,.5-.5v-7A.5.5,0,0,0,2.5,9Z" />
			<path d="M4.5,10H8.443A6.14249,6.14249,0,0,1,13.5,7.35q.25275,0,.5.0205V5a3,3,0,0,0-3-3H7A3,3,0,0,0,4,5V9.5A.5.5,0,0,0,4.5,10Z" />
			<path d="M7.35,13.5A6.128,6.128,0,0,1,7.88,11H4.5a.5.5,0,0,0-.5.5v3a.5.5,0,0,0,.5.5H7.5345A6.15037,6.15037,0,0,1,7.35,13.5Z" />
			<path d="M13.55,9.1A4.45,4.45,0,1,0,18,13.55,4.45,4.45,0,0,0,13.55,9.1Zm2.5,4.7a.25.25,0,0,1-.25.25H14.05V15.8a.25.25,0,0,1-.25.25h-.5a.25.25,0,0,1-.25-.25V14.05H11.3a.25.25,0,0,1-.25-.25v-.5a.25.25,0,0,1,.25-.25h1.75V11.3a.25.25,0,0,1,.25-.25h.5a.25.25,0,0,1,.25.25v1.75H15.8a.25.25,0,0,1,.25.25Z" />
		</svg>
	)
}
